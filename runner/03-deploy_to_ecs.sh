...
LATEST_DOCKER_IMAGE=`aws ecr list-images --repository-name $CONTAINER_IMAGE_NAME --query "imageIds[0].imageTag" --output text`
...
GROUP_NAME=$BLUE_SYM  DOCKER_IMAGE=$CONTAINER_IMAGE_NAME DOCKER_TAG=$LATEST_DOCKER_IMAGE ecspresso --config="$PWD/../ecspresso/ecspresso.yml" deploy --tasks=1
